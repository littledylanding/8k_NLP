...
def plot_mean_cum_ret(data, items, up=0.75, down=0.25):
    for item in items:
        print(item)
        pct_changes = data[data['Items'] == item].groupby(['Ticker', 'Filing Date'])[['Ret']].apply(lambda x: x.iloc[10:])
        median = [0]
        upper = [0]
        lower = [0]
        max_len = max(pct_changes.groupby(level=[0, 1]).apply(len))
        mid = max_len-2
        x = np.array(range(mid + 3))
        mean = [0]
        for i in range(max_len):
            nth_values = pct_changes.groupby(level=[0, 1])['Ret'].apply(
